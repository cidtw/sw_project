...
- 내부 DB 검색 결과가 없을 때 Google 사이트 제한 검색 링크 제공
- 크롤된 공고, 맞춤 공고, 검색한 공고를 사용자별로 스크랩 저장
...
- `일반 공고 검색`: 제목, 회사명, 지역, 자격요건, 우대요건, 직무기술서, 키워드까지 포함한 전체 텍스트 검색을 기본값으로 사용하고, 검색 결과를 페이지 단위 ephemeral 메시지로 표시합니다. 내부 DB 결과가 없으면 Google 사이트 제한 검색 링크를 제공합니다.
- `스크랩`: 최신 업데이트, 맞춤 추천, 일반 검색 결과에 붙은 `스크랩` 버튼으로 공고를 저장하고, `스크랩된 공고 보기`에서 다시 확인합니다.
